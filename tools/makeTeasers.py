...
for lang in LANGS:
    # structure has been updated, we need to scan project and veranstaltungen and make a combined list, with teaser text and image. make sure to have 
    # the proper links for the language and for projekte and veranstaltungen.
    for dir in ["projekte","veranstaltungen"]:
        prjdir = f"../content/{lang}/{dir}"
        print(prjdir)
        ...
        for fl in files:
            ...
            with open(os.sep.join([prjdir,fl])) as f:
                ...
                # all images remain in the projects folder, so we can use the project name to get the image
                if yml.get("imgname") != None:
                    prjimg = f"{LABURL}/projects/{yml['imgname']}"
                # make project link, lang specific and separate for projects and veranstaltungen
                if lang == LANGS[0]:
                    prjurl = f"{LABURL}/{dir}/{prjname}"
                else:
                    prjurl = f"{LABURL}/{lang}/{dir}/{prjname}"
