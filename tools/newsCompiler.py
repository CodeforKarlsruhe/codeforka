    ...
    <mj-section mj-class="webview">
      <mj-column>
        <mj-text  mj-class="webview" align="center" color="#999999">
          <a href="https://ok-lab-karlsruhe.de/news" style="color: #999999;text-decoration: none;">Zur Webdarstellung</a>
        </mj-text>
      </mj-column>
    </mj-section>
    ...
    <mj-section background-color="#444F60">
      <mj-column>
        <mj-text mj-class="text" color="#ffffff">
          Zum Abmelden von diesem Newsletter klicken Sie bitte auf diesen
          <a href="https://ok-lab-karlsruhe.de/php/action.php/?lang=de&remove={{{remove}}}" style="color: #98a9c3; text-decoration:none;" Title="Abmeldelink">Link</a>
        </mj-text>
      </mj-column>
    </mj-section>
  </mj-wrapper>
...
pfile = config["crypt"]["pass"].replace("\"","") # /home/kugel/daten/work/projekte/okLabs/okLab/2020/cert/pass.txt"
cfile = config["crypt"]["key"].replace("\"","")  # /home/kugel/daten/work/projekte/okLabs/okLab/2020/cert/oklab-key.pem"
...
try:
    privkey = load_pem_private_key(keybuf, pwd, default_backend())
    #print("Key: ",privkey, ", size: ",privkey.key_size)
except:
...
for a in addrData:
    bincipher =  binascii.unhexlify(a["email"])
    email = privkey.decrypt(
        bincipher,
        padding.PKCS1v15()
    ).decode("utf-8")
    bincipher =  binascii.unhexlify(a["code"])
    code = privkey.decrypt(
        bincipher,
        padding.PKCS1v15()
    ).decode("utf-8")
    addr.append({"email":email,"code":code})
    # print("addr: ",email,",",code)
...
for a in addr:
    # render remove link
    context = {"remove":a["code"]}
    phtml = pystache.render(html,context)
